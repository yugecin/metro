...
"p.z = abs(p.z+1.8) - .8;"
"float a=su("
"length(max(abs(p+vec3(0.,0.,.5)) - vec3(.6,5.,.7),0.)), "
"length(max(abs(p) - vec3(1.,5.,.2),0.)) "
",.3"
");"
"if (a<.01)s=1;"
"return a;"
...
"min("
"length(max(abs(p) - vec3(1.6,1.7,.7),0.)), "
"min("
"railrail(p),"
"length(max(abs(q)-vec3(4.5,2.5,.5),0.)) "
")"
")"
");"
"}"
...
"float rail(vec3 p) {"
"return min("
"length(max(abs(p) - vec3(13.,2.5,.5),0.)), "
"mirroredrail(p)"
");"
"}"
...
");"
"}"
"float topsupport(vec2 p) { "
"p.y += 63;"
"float v=length(max(abs(p)-vec2(1,5),0)),"
"h=length(max(abs(vec3(mod(op.x,20)-10,p.xy))-vec3(1,3,5),0)),"
"v2;"
"p.y=abs(p.y)-5.5;"
...
"w=vec2(min(dot(p,vec3(-1,0,0))+103,dot(p,vec3(1,0,0))+263),3), "
...
"}"
"float flopine_shade;"
"vec3 march(vec3 o,vec3 v,int s){ "
"vec3 r=vec3(0);"
